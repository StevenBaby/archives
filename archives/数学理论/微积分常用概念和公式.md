...
$$
\Delta = B^2 - AC \left\{\begin{aligned}
&< 0 \Rightarrow 极值 \left\{
    \begin{aligned}
    &A < 0 \Rightarrow 极大值 \\
    &A > 0 \Rightarrow 极小值
    \end{aligned}
...
求目标函数 $u = f(x, y, z)$，在条件 $\left\{\begin{aligned} &\varphi(x,y,z)=0 \\&\psi(x, y, z) = 0 \\\end{aligned}\right.$ 下的最值，则：
...
$$
\begin{aligned}
\Gamma(\alpha) &= \int_0^{+\infty} t^{\alpha-1}e^{-t} dt \ (x>0) \\
\Gamma(\alpha) &= 2\int_0^{+\infty} t^{2\alpha-1}e^{-t^2} dt \ (x>0)
\end{aligned}
$$
