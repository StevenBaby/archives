...
可以使用两个指令使整数值向左移位一
SAL （向左犹术移位）和SHL （向左逻钳移位）。这两个指令执行相同的操作，井且是可以互换的，它们有 3 种不同格式：
...
有两个向右移位指令。`SHR` 指令情空移位造成的空位，所以它只能用于对无符号整数进行移位操作，`SAR` 指令根据整数的符号位，要么清空，要么设置移位造成的空位，对于负数，空位被设置为 1，对于正数，它们被设位 0.
